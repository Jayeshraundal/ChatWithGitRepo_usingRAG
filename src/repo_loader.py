...
class RepositoryLoader:
        ...
        for excluded in EXCLUDED_DIRS:
            ...
            if excluded_path.exists():
                if excluded_path.is_dir():
                    shutil.rmtree(excluded_path, onerror=RepositoryLoader._remove_readonly)
                else:
                    try:
    ...
    def clone_repo(repo_url: str) -> Path:
        repo_name = RepositoryLoader.get_repo_name(repo_url)
        ...
        RepositoryLoader.clean_repo(repo_path)
        return repo_path
